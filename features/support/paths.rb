module NavigationHelpers
  ...
  def path_to(page_name)
    ...
    when /the messageboard "([^\"]+)"/i
      messageboard = Messageboard.find_by_name($1)
      site_messageboard_topics_path(@site.slug, messageboard)

    when /the add a new thread page for "([^\"]+)"/i
      messageboard = Messageboard.find_by_name($1)
      new_site_messageboard_topic_path(@site.slug, messageboard)
    ...
    when /edit the latest thread/i
      ...
      edit_site_messageboard_topic_path(@site.slug, messageboard, topic)

    when /the most recently updated thread on "([^\"]+)"/i
      messageboard = Messageboard.find_by_name($1)
      topic = messageboard.topics.latest.first
      site_messageboard_topic_path(@site.slug, messageboard, topic)
